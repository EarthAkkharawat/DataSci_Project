...
def fetch_data_from_traffyapi(ti):
    ...
    url = "https://publicapi.traffy.in.th/share/teamchadchart/search?start="+formatted_date+"&end="+formatted_date+"&sort=ASC"
    ...
    if "results" in data:
        ...
        for item in data.get("results", []):
            ...
            timestamp = item.get("timestamp").split(" ")[0]  # Extract date from timestamp
...
def convert_images_to_jpg(ti):
    ...
    new_directory = os.path.join(os.path.dirname(current_directory), 'daily_images')
...
def feed_data_to_model(ti):
    ...
    realtime_uploaded_images = TraffyFondueImages('../daily_images', transform, image_names_list)
    realtime_uploaded_images_loader = torch.utils.data.DataLoader( realtime_uploaded_images, batch_size=32, shuffle=False)
    ...
    ti.xcom_push(key='model_prediction', value=predict)

def send_prediction_to_vis(ti):
    model_prediction = ti.xcom_pull(key='model_prediction', task_ids = 'feed_data_to_model')
    payload = ti.xcom_pull(key='traffyapi_data', task_ods = 'fetch_data')
